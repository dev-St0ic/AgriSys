...
/**
 * CSRF Token Refresh
 * Provides fresh CSRF token for AJAX requests
 */
...
/**
 * Document Download
 * Allows downloading of whitelisted documents (e.g., Barangay Certification)
 */
 ...
 * Handles submissions from public-facing application forms
 */
...
// Alternative BoatR submission route (matches JavaScript call)
...
/**
 * Password Change Verification Route (Public)
 * Handles email-based password change confirmations
 */
...
/**
 * Email Change Confirmation Route (Public)
 * Handles email-based email address change confirmations
 */
 ...
 * Basic admin routes without email verification requirement
 */
...
/**
 * Service Routes
 * For frontend navigation to different service sections
 */
...
/**
 * FishR Number Validation (Public)
 * Validates FishR numbers for BoatR applications
 */
...
Route::middleware('admin')->group(function () {
     ...
     * Allows admins to edit their own profile
     */
...
// ==============================================
// API ROUTES FOR AJAX REQUESTS (Admin)
// ==============================================

/**
 * Admin API Routes
 * AJAX endpoints for admin panel operations
 */
...
if (config('app.debug')) {
     ...
     * Shows all registered routes in JSON format
     */
    ...
    /**
     * Debug: Test RSBSA submission route
     */
    ...
    /**
     * Debug: Test BoatR submission route
     */
...
/*
|--------------------------------------------------------------------------
| User Registration Management Routes (Admin)
|--------------------------------------------------------------------------
*/

/**
 * User Registration Management
 * Admin routes for managing user accounts and verifications
 */
...
/*
|--------------------------------------------------------------------------
| Authentication Routes (User Registration System)
|--------------------------------------------------------------------------
*/
...
/*
|--------------------------------------------------------------------------
| Session Check Endpoint (Critical for Session Management)
|--------------------------------------------------------------------------
*/
 ...
 * 3. Updates last_activity timestamp (resets 30-minute inactivity timer)
 * 4. Returns session status to JavaScript
 */
Route::get('/api/user/session-check', function (\Illuminate\Http\Request $request) {
    ...
    // CRITICAL: Update last activity timestamp
    // This resets the inactivity counter and keeps the session alive
...
/**
 * Recycle Bin
 * Manages soft-deleted records across all modules
 */
...
/**
 * User Dashboard
 * Routes for logged-in users (farmers/fisherfolks)
 * Protected by UserSession and CheckSessionExpiration middleware
 */
Route::middleware(['web', App\Http\Middleware\UserSession::class, App\Http\Middleware\CheckSessionExpiration::class])->group(function () {
    ...
    Route::prefix('api/user')->group(function () {
        ...
        // Session refresh endpoint (syncs session with database)
        Route::post('/refresh-session', [UserRegistrationController::class, 'refreshUserSession'])->name('api.user.refresh-session');

        // Get all applications (RSBSA, Seedlings, FishR, BoatR, Training)
...
/**
 * SMS Notifications
 * Handles SMS sending functionality (Admin only)
 */
Route::prefix('api/sms')->group(function () {
    Route::middleware(['auth.session', 'check.admin'])->group(function () {
        Route::post('/send-notification', [SmsNotificationController::class, 'sendNotification'])->name('api.sms.send-notification');
        Route::get('/service-status', [SmsNotificationController::class, 'getServiceStatus'])->name('api.sms.service-status');
    });
});
...
/*
|--------------------------------------------------------------------------
| Storage File Serving (Fallback)
|--------------------------------------------------------------------------
*/

/**
 * Storage File Serving
 * Fallback for hosts without symlink support
