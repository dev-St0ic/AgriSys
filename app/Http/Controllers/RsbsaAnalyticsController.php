        ...
        try {
            ...
            if (auth()->check()) {
                activity()
                    ...
                    ->event('viewed')
                    ->log('viewed - RsbsaAnalytics Dashboard');
            }

            // Date range filter with better defaults
            $earliestRecord = \App\Models\RsbsaApplication::min('created_at');
            $defaultStart   = $earliestRecord
                ? \Carbon\Carbon::parse($earliestRecord)->format('Y-m-d')
