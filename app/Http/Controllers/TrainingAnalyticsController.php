...
class TrainingAnalyticsController extends Controller
{
      ...
      public function export(Request $request)
    {
        try {
            ...
            $filename = 'training-analytics-' . $startDate . '-to-' . $endDate . '.csv';
            ...
            $monthlyTrends       = $this->getMonthlyTrends($startDate, $endDate);
            $trainingTypeAnalysis= $this->getTrainingTypeAnalysis($startDate, $endDate);
            $processingTime      = $this->getProcessingTimeAnalysis($startDate, $endDate);
            $documentAnalysis    = $this->getDocumentAnalysis($startDate, $endDate);
            $performanceMetrics  = $this->getPerformanceMetrics($startDate, $endDate);
            $demographicAnalysis = $this->getDemographicAnalysis($startDate, $endDate);
            $barangayAnalysis    = $this->getBarangayAnalysis($startDate, $endDate);
            ...
            $callback = function () use (
                ...
                $trainingTypeAnalysis, $processingTime, $documentAnalysis,
                $performanceMetrics, $demographicAnalysis, $barangayAnalysis
            ) {
                $file = fopen('php://output', 'w');

                // ── Export Info ───────────────────────────────────────
                fputcsv($file, ['TRAINING ANALYTICS EXPORT']);
                ...
                fputcsv($file, ['Unique Training Types',    $overview['unique_training_types']]);
                fputcsv($file, ['With Contact Number',      $overview['with_contact_number']]);
                fputcsv($file, ['Contact Provision Rate',   $overview['contact_provision_rate'] . '%']);
                fputcsv($file, []);
                ...
                fputcsv($file, ['Month', 'Total Applications', 'Approved', 'Rejected', 'Pending', 'With Documents', 'Unique Training Types']);
                foreach ($monthlyTrends as $trend) {
                    fputcsv($file, [
                        ...
                        $trend->unique_training_types,
                    ]);
                }
                fputcsv($file, []);

                // ── Training Type Analysis ────────────────────────────
                fputcsv($file, ['TRAINING TYPE ANALYSIS']);
                fputcsv($file, ['Training Type', 'Total Applications', 'Approved', 'Rejected', 'Pending', 'Unique Applicants', 'With Documents', 'Avg Processing Days']);
                foreach ($trainingTypeAnalysis as $t) {
                    fputcsv($file, [
                        $t->training_type,
                        ...
                        $t->unique_applicants,
                        $t->with_documents,
                        round($t->avg_processing_days ?? 0, 2),
                    ]);
                }
                fputcsv($file, []);
                ...
                fputcsv($file, ['DEMOGRAPHIC ANALYSIS - TOP NAMES']);
                fputcsv($file, ['First Name', 'Count']);
                foreach ($demographicAnalysis['common_names'] as $name) {
                    fputcsv($file, [$name->first_name, $name->count]);
                }
                fputcsv($file, []);
