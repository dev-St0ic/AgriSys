...
class DSSDataService
{
    ...
    /**
     * Resolve start/end Carbon dates from either monthly or quarterly params.
     * Always returns [$startDate, $endDate, $periodLabel, $cacheKeySuffix].
     */
    private function resolvePeriod(
        ...
        if ($periodMode === 'quarterly') {
            $quarter    = (int)($quarter ?? ceil(now()->month / 3));
            $quarter    = max(1, min(4, $quarter));
            [$mStart, $mEnd] = self::QUARTER_MONTHS[$quarter];

            $startDate   = Carbon::createFromDate($year, $mStart, 1)->startOfMonth();
            $endDate     = Carbon::createFromDate($year, $mEnd, 1)->endOfMonth();
            $periodLabel = "Q{$quarter} {$year} (" . $startDate->format('M') . '–' . $endDate->format('M Y') . ')';
            $cacheSuffix = "Q{$quarter}_{$year}";
        } else {
            $month      = $month ?? now()->format('m');
            $startDate  = Carbon::createFromDate($year, $month, 1)->startOfMonth();
            $endDate    = Carbon::createFromDate($year, $month, 1)->endOfMonth();
            $periodLabel = $startDate->format('F Y');
            $cacheSuffix = "{$year}_{$month}";
        }

        return [$startDate, $endDate, $periodLabel, $cacheSuffix, $periodMode];
    }

    /**
     * Collect comprehensive data for DSS analysis (Supplies).
     */
    public function collectMonthlyData(
        string $month      = null,
        string $year       = null,
        string $periodMode = 'monthly',
        string $quarter    = null
        ...
        [$startDate, $endDate, $periodLabel, $cacheSuffix] =
            $this->resolvePeriod($periodMode, $month, $year, $quarter);

        $cacheKey = "dss_data_{$cacheSuffix}";

        return Cache::remember($cacheKey, 1800, function () use ($startDate, $endDate, $periodLabel, $periodMode) {
            return [
                'period' => [
                    'month'       => $periodLabel,
    ...
    /**
     * Clear cached DSS data for a specific period.
     */
    public function clearCache(
        ...
        [,,,,$cacheSuffix] = $this->resolvePeriod($periodMode, $month, $year, $quarter);
        Cache::forget("dss_data_{$cacheSuffix}");
    }

    /**
     * Clear all DSS related cache (monthly + quarterly for past 3 months/quarters).
     */
    public function clearAllCache(): void
    {
        ...
        for ($i = 0; $i < 3; $i++) {
            ...
            $this->clearCache($date->format('m'), $date->format('Y'), 'monthly');
        }
        // Also clear current and previous 2 quarters
        for ($i = 0; $i < 3; $i++) {
            $date    = $currentDate->copy()->subMonths($i * 3);
            $quarter = (string) ceil($date->month / 3);
            $this->clearCache(null, $date->format('Y'), 'quarterly', $quarter);
        }
    }

    // ─────────────────────────────────────────────────────────────────
    // SUPPLIES — private helpers (unchanged)
    // ─────────────────────────────────────────────────────────────────
    ...
    private function getSeedlingRequestsData(Carbon $startDate, Carbon $endDate): array
    {
        ...
        return [
            ...
            'approval_rate'            => $requestStats->total_requests > 0
                ? round(($requestStats->approved_requests / $requestStats->total_requests) * 100, 2)
                : 0,
        ];
    }
    ...
    private function getDemandAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $demandQuery = DB::table('seedling_requests as sr')
            ...
            ->select('sri.item_name', 'ci.name as category_name',
        ...
        $demandData = $demandQuery->map(fn ($item) => [
            'name'            => $item->item_name,
        ...
        $categoryAnalysis = collect($demandData)->groupBy('category')->map(fn ($items) => [
    ...
    private function getBarangayAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        ...
        return [
            ...
            'barangay_details'           => $requests->map(fn ($item) => [
                'name'           => $item->barangay,
    ...
    private function getFulfillmentAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $approvedRequests    = SeedlingRequest::whereBetween('created_at', [$startDate, $endDate])
            ->whereIn('status', ['approved', 'partially_approved'])->get();
    ...
    private function getShortageAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        ...
        foreach ($demandData['top_requested_items'] as $demandItem) {
            $matchingStock = collect($supplyData['stock_distribution'])
                ->first(fn ($stock) =>
                    str_contains(strtolower($stock['name']), strtolower($demandItem['name'])) ||
                    str_contains(strtolower($demandItem['name']), strtolower($stock['name']))
                );
    ...
    public function collectTrainingData(
        ...
        $cacheKey = "dss_training_data_{$cacheSuffix}";
    ...
    private function getTrainingByBarangay(Carbon $startDate, Carbon $endDate): array
    {
        $byBarangay = TrainingApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('barangay')->selectRaw('COUNT(*) as count')
            ->groupBy('barangay')->orderByDesc('count')
            ->get()->map(fn ($item) => ['barangay' => $item->barangay, 'applications' => $item->count])->toArray();
    ...
    private function getTrainingApprovalAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        ...
        return [
            'average_decision_time' => $applications->isNotEmpty()
                ? round($applications->avg(fn ($a) => $a->created_at->diffInHours($a->status_updated_at)), 2) . ' hours'
                : '0 hours',
            'fastest_approval'      => $applications->where('status', 'approved')->isNotEmpty()
                ? round($applications->where('status', 'approved')->min(fn ($a) => $a->created_at->diffInHours($a->status_updated_at)), 2) . ' hours'
                : 'N/A',
            'slowest_approval'      => $applications->where('status', 'approved')->isNotEmpty()
                ? round($applications->where('status', 'approved')->max(fn ($a) => $a->created_at->diffInHours($a->status_updated_at)), 2) . ' hours'
                : 'N/A',
        ];
    }
    ...
    {
        // For quarterly, compare vs previous quarter; for monthly, vs previous month
        $spanDays      = $startDate->diffInDays($endDate) + 1;
        $prevStartDate = $startDate->copy()->subDays($spanDays);
    ...
    public function collectRsbsaData(
        string $month      = null,
        string $year       = null,
        string $periodMode = 'monthly',
        string $quarter    = null
        ...
        $cacheKey = "dss_rsbsa_data_{$cacheSuffix}";
    ...
    private function getRsbsaByCommodity(Carbon $startDate, Carbon $endDate): array
    {
        $byCommodity = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->whereNotNull('commodity')->where('commodity', '!=', '')
            ->select('commodity')
            ->selectRaw('COUNT(*) as count, SUM(farmer_land_area) as total_land, AVG(farmer_land_area) as avg_land')
            ->groupBy('commodity')->orderByDesc('count')
            ->get()->map(fn ($item) => [
        ...
        return ['distribution' => $byCommodity, 'top_commodities' => array_slice($byCommodity, 0, 5), 'total_commodity_types' => count($byCommodity)];
    }
    ...
    private function getRsbsaByLivelihood(Carbon $startDate, Carbon $endDate): array
    {
        $byLivelihood = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('main_livelihood')->selectRaw('COUNT(*) as count')
            ->groupBy('main_livelihood')->orderByDesc('count')
            ->get()->map(fn ($item) => ['livelihood' => $item->main_livelihood, 'count' => $item->count])->toArray();

        return ['distribution' => $byLivelihood, 'primary_livelihood' => $byLivelihood[0] ?? null];
    }
    ...
    private function getRsbsaByBarangay(Carbon $startDate, Carbon $endDate): array
    {
        $byBarangay = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('barangay')->selectRaw('COUNT(*) as count, SUM(farmer_land_area) as total_land')
            ->groupBy('barangay')->orderByDesc('count')
            ->get()->map(fn ($item) => ['barangay' => $item->barangay, 'farmers' => $item->count, 'total_land_area' => round($item->total_land ?? 0, 2)])->toArray();

        return ['distribution' => $byBarangay, 'top_barangays' => array_slice($byBarangay, 0, 5), 'total_barangays_covered' => count($byBarangay)];
    }

    private function getRsbsaDemographics(Carbon $startDate, Carbon $endDate): array
    {
        $d = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->selectRaw('SUM(CASE WHEN sex = "male" THEN 1 ELSE 0 END) as male_count, SUM(CASE WHEN sex = "female" THEN 1 ELSE 0 END) as female_count, COUNT(*) as total')
            ->first();

        return [
            'male_count'        => $d->male_count ?? 0,
            'female_count'      => $d->female_count ?? 0,
            'total'             => $d->total ?? 0,
            'male_percentage'   => $d->total > 0 ? round(($d->male_count / $d->total) * 100, 2) : 0,
            'female_percentage' => $d->total > 0 ? round(($d->female_count / $d->total) * 100, 2) : 0,
        ];
    }
    ...
    private function getRsbsaLandAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $landData = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->whereNotNull('farmer_land_area')->select('farmer_land_area')->get();

        return [
            'small_farms'    => $landData->where('farmer_land_area', '<', 1)->count() . ' (< 1 ha)',
            'medium_farms'   => $landData->where('farmer_land_area', '>=', 1)->where('farmer_land_area', '<', 3)->count() . ' (1-3 ha)',
            'large_farms'    => $landData->where('farmer_land_area', '>=', 3)->count() . ' (≥ 3 ha)',
    ...
    private function getRsbsaFarmerDetails(Carbon $startDate, Carbon $endDate): array
    {
        $byTypeOfFarm = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Farmer')->whereNotNull('farmer_type_of_farm')->where('farmer_type_of_farm', '!=', '')
            ->select('farmer_type_of_farm')->selectRaw('COUNT(*) as count')->groupBy('farmer_type_of_farm')->orderByDesc('count')
            ->get()->map(fn ($i) => ['type' => $i->farmer_type_of_farm, 'count' => $i->count])->toArray();

        $byLandOwnership = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Farmer')->whereNotNull('farmer_land_ownership')->where('farmer_land_ownership', '!=', '')
            ->select('farmer_land_ownership')->selectRaw('COUNT(*) as count')->groupBy('farmer_land_ownership')->orderByDesc('count')
            ->get()->map(fn ($i) => ['ownership' => $i->farmer_land_ownership, 'count' => $i->count])->toArray();

        $bySpecialStatus = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Farmer')->whereNotNull('farmer_special_status')->where('farmer_special_status', '!=', '')
            ->select('farmer_special_status')->selectRaw('COUNT(*) as count')->groupBy('farmer_special_status')->orderByDesc('count')
            ->get()->map(fn ($i) => ['status' => $i->farmer_special_status, 'count' => $i->count])->toArray();

        return [
            'total_farmers'    => RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])->where('main_livelihood', 'Farmer')->count(),
    ...
    private function getRsbsaFarmerCrops(Carbon $startDate, Carbon $endDate): array
    {
        $byCrop = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Farmer')->whereNotNull('farmer_crops')->where('farmer_crops', '!=', '')
            ->select('farmer_crops')->selectRaw('COUNT(*) as count, SUM(farmer_land_area) as total_land')
            ->groupBy('farmer_crops')->orderByDesc('count')
            ->get()->map(fn ($i) => ['crop' => $i->farmer_crops, 'count' => $i->count, 'total_land_area' => round($i->total_land ?? 0, 2)])->toArray();

        $otherCrops = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('farmer_crops', 'Other Crops')->whereNotNull('farmer_other_crops')->where('farmer_other_crops', '!=', '')
            ->select('farmer_other_crops')->selectRaw('COUNT(*) as count')->groupBy('farmer_other_crops')->orderByDesc('count')
            ->get()->map(fn ($i) => ['crop' => $i->farmer_other_crops, 'count' => $i->count])->toArray();

        return ['distribution' => $byCrop, 'top_crops' => array_slice($byCrop, 0, 5), 'total_crop_types' => count($byCrop), 'other_crops_specified' => $otherCrops];
    }
    ...
    private function getRsbsaFarmworkerType(Carbon $startDate, Carbon $endDate): array
    {
        $byType = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Farmworker/Laborer')->whereNotNull('farmworker_type')->where('farmworker_type', '!=', '')
            ->select('farmworker_type')->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count')
            ->groupBy('farmworker_type')->orderByDesc('count')
            ->get()->map(fn ($i) => ['type' => $i->farmworker_type, 'count' => $i->count, 'approved' => $i->approved_count,
                'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0])->toArray();

        $otherTypes = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('farmworker_type', 'Others')->whereNotNull('farmworker_other_type')->where('farmworker_other_type', '!=', '')
            ->select('farmworker_other_type')->selectRaw('COUNT(*) as count')->groupBy('farmworker_other_type')->orderByDesc('count')
            ->get()->map(fn ($i) => ['type' => $i->farmworker_other_type, 'count' => $i->count])->toArray();

        return [
            'total_farmworkers'      => RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])->where('main_livelihood', 'Farmworker/Laborer')->count(),
            'distribution'           => $byType,
    ...
    private function getRsbsaFisherfolkActivity(Carbon $startDate, Carbon $endDate): array
    {
        $byActivity = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Fisherfolk')->whereNotNull('fisherfolk_activity')->where('fisherfolk_activity', '!=', '')
            ->select('fisherfolk_activity')->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count')
            ->groupBy('fisherfolk_activity')->orderByDesc('count')
            ->get()->map(fn ($i) => ['activity' => $i->fisherfolk_activity, 'count' => $i->count, 'approved' => $i->approved_count,
                'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0])->toArray();

        $otherActivities = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('fisherfolk_activity', 'Others')->whereNotNull('fisherfolk_other_activity')->where('fisherfolk_other_activity', '!=', '')
            ->select('fisherfolk_other_activity')->selectRaw('COUNT(*) as count')->groupBy('fisherfolk_other_activity')->orderByDesc('count')
            ->get()->map(fn ($i) => ['activity' => $i->fisherfolk_other_activity, 'count' => $i->count])->toArray();

        return [
            'total_fisherfolk'           => RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])->where('main_livelihood', 'Fisherfolk')->count(),
    ...
    private function getRsbsaAgriyouthAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $demographics = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ...
            ->selectRaw('SUM(CASE WHEN sex = "Male" THEN 1 ELSE 0 END) as male_count, SUM(CASE WHEN sex = "Female" THEN 1 ELSE 0 END) as female_count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved, COUNT(*) as total')
            ->first();

        $byFarmingHousehold = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Agri-youth')->whereNotNull('agriyouth_farming_household')->where('agriyouth_farming_household', '!=', '')
            ->select('agriyouth_farming_household')->selectRaw('COUNT(*) as count')->groupBy('agriyouth_farming_household')->orderByDesc('count')
            ->get()->map(fn ($i) => ['value' => $i->agriyouth_farming_household, 'count' => $i->count])->toArray();

        $byTraining = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Agri-youth')->whereNotNull('agriyouth_training')->where('agriyouth_training', '!=', '')
            ->select('agriyouth_training')->selectRaw('COUNT(*) as count')->groupBy('agriyouth_training')->orderByDesc('count')
            ->get()->map(fn ($i) => ['training' => $i->agriyouth_training, 'count' => $i->count])->toArray();

        $byParticipation = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->where('main_livelihood', 'Agri-youth')->whereNotNull('agriyouth_participation')->where('agriyouth_participation', '!=', '')
            ->select('agriyouth_participation')->selectRaw('COUNT(*) as count')->groupBy('agriyouth_participation')->orderByDesc('count')
            ->get()->map(fn ($i) => ['participation' => $i->agriyouth_participation, 'count' => $i->count])->toArray();

        return [
            'total_agriyouth'       => RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])->where('main_livelihood', 'Agri-youth')->count(),
            'male_count'            => $demographics->male_count ?? 0,
            'female_count'          => $demographics->female_count ?? 0,
            'approved'              => $demographics->approved ?? 0,
            'approval_rate'         => ($demographics->total ?? 0) > 0 ? round(($demographics->approved / $demographics->total) * 100, 2) : 0,
    ...
    {
        $applications = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])->whereNotNull('reviewed_at')->get();

        return [
            'average_review_time' => $applications->isNotEmpty()
                ? round($applications->avg(fn ($a) => $a->created_at->diffInHours($a->reviewed_at)), 2) . ' hours' : '0 hours',
            'total_reviewed'      => $applications->count(),
            'fastest_review'      => $applications->isNotEmpty()
                ? round($applications->min(fn ($a) => $a->created_at->diffInHours($a->reviewed_at)), 2) . ' hours' : 'N/A',
        ];
    }
    ...
    private function getRsbsaTrends(Carbon $startDate, Carbon $endDate): array
    {
        ...
        $byLivelihoodCurrent = RsbsaApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('main_livelihood')->selectRaw('COUNT(*) as count')->groupBy('main_livelihood')->orderByDesc('count')
            ->get()->map(fn ($i) => ['livelihood' => $i->main_livelihood, 'count' => $i->count])->toArray();
    ...
    public function collectFishrData(
        string $month      = null,
        string $year       = null,
        string $periodMode = 'monthly',
        string $quarter    = null
        ...
        $cacheKey = "dss_fishr_data_{$cacheSuffix}";
    ...
    private function getFishrByLivelihood(Carbon $startDate, Carbon $endDate): array
    {
        $byLivelihood = FishrApplication::whereBetween('created_at', [$startDate, $endDate])
            ...
            ->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count')
            ->groupBy('main_livelihood')->orderByDesc('count')
            ->get()->map(fn ($i) => ['livelihood' => $i->main_livelihood ?? 'Not Specified', 'total' => $i->count,
                'approved' => $i->approved_count, 'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0])->toArray();

        return ['distribution' => $byLivelihood, 'most_common' => collect($byLivelihood)->sortByDesc('total')->take(3)->values()->toArray(), 'total_livelihood_types' => count($byLivelihood)];
    }
    ...
    private function getFishrBySecondaryLivelihood(Carbon $startDate, Carbon $endDate): array
    {
        $bySecondary = FishrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->whereNotNull('secondary_livelihood')->where('secondary_livelihood', '!=', '')
            ->select('secondary_livelihood')
            ->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count')
            ->groupBy('secondary_livelihood')->orderByDesc('count')
            ->get()->map(fn ($i) => ['livelihood' => $i->secondary_livelihood ?? 'Not Specified', 'total' => $i->count,
                'approved' => $i->approved_count, 'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0])->toArray();

        return [
            'distribution'         => $bySecondary,
            'total_with_secondary' => FishrApplication::whereBetween('created_at', [$startDate, $endDate])->whereNotNull('secondary_livelihood')->where('secondary_livelihood', '!=', '')->count(),
    ...
    private function getFishrByBarangay(Carbon $startDate, Carbon $endDate): array
    {
        $byBarangay = FishrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('barangay')->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved')
            ->groupBy('barangay')->orderByDesc('count')
            ->get()->map(fn ($i) => ['barangay' => $i->barangay ?? 'Unknown', 'applications' => $i->count, 'approved' => $i->approved])->toArray();
    ...
    {
        $d = FishrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->selectRaw('SUM(CASE WHEN sex = "male" THEN 1 ELSE 0 END) as male_count, SUM(CASE WHEN sex = "female" THEN 1 ELSE 0 END) as female_count, COUNT(*) as total')
            ->first();
    ...
    {
        $applications = FishrApplication::whereBetween('created_at', [$startDate, $endDate])->whereNotNull('status_updated_at')->get();

        return [
            'average_processing_time' => $applications->isNotEmpty()
                ? round($applications->avg(fn ($a) => $a->created_at->diffInHours($a->status_updated_at)), 2) . ' hours' : '0 hours',
            'total_processed'         => $applications->count(),
            'fastest_processing'      => $applications->isNotEmpty()
                ? round($applications->min(fn ($a) => $a->created_at->diffInHours($a->status_updated_at)), 2) . ' hours' : 'N/A',
            'slowest_processing'      => $applications->isNotEmpty()
                ? round($applications->max(fn ($a) => $a->created_at->diffInHours($a->status_updated_at)), 2) . ' hours' : 'N/A',
        ];
    }
    ...
    private function getFishrTrends(Carbon $startDate, Carbon $endDate): array
    {
        ...
        return ['current_month_total' => $currentCount, 'previous_month_total' => $previousCount,
            'change_percentage' => $change, 'trend' => $change > 0 ? 'increasing' : ($change < 0 ? 'decreasing' : 'stable')];
    }
    ...
    public function collectBoatrData(
        string $month      = null,
        string $year       = null,
        string $periodMode = 'monthly',
        string $quarter    = null
        ...
        $cacheKey = "dss_boatr_data_{$cacheSuffix}";
    ...
    private function getBoatrStats(Carbon $startDate, Carbon $endDate): array
    {
        ...
        return [
            ...
            'approval_rate'             => $stats->total_applications > 0 ? round(($stats->approved / $stats->total_applications) * 100, 2) : 0,
            'inspection_rate'           => $stats->total_applications > 0 ? round(($stats->inspections_completed / $stats->total_applications) * 100, 2) : 0,
            'document_verification_rate'=> $stats->total_applications > 0 ? round(($stats->documents_verified / $stats->total_applications) * 100, 2) : 0,
    ...
    private function getBoatrByBoatType(Carbon $startDate, Carbon $endDate): array
    {
        $byBoatType = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('boat_type')
            ->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count, AVG(boat_length) as avg_length, AVG(engine_horsepower) as avg_hp')
            ->groupBy('boat_type')->orderByDesc('count')
            ->get()->map(fn ($i) => ['boat_type' => $i->boat_type ?? 'Not Specified', 'total' => $i->count,
                'approved' => $i->approved_count, 'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0,
                'avg_length' => round($i->avg_length ?? 0, 2), 'avg_horsepower' => round($i->avg_hp ?? 0, 2)])->toArray();

        $byClassification = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->whereNotNull('boat_classification')->where('boat_classification', '!=', '')
            ->select('boat_classification')->selectRaw('COUNT(*) as count')->groupBy('boat_classification')->orderByDesc('count')
            ->get()->map(fn ($i) => ['classification' => $i->boat_classification, 'total' => $i->count])->toArray();

        return ['distribution' => $byBoatType, 'most_common' => collect($byBoatType)->sortByDesc('total')->take(3)->values()->toArray(),
            'total_boat_types' => count($byBoatType), 'by_classification' => $byClassification];
    }
    ...
    private function getBoatrEngineAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $engineTypes = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->select('engine_type')->selectRaw('COUNT(*) as count, AVG(engine_horsepower) as avg_hp')
            ->groupBy('engine_type')->orderByDesc('count')
            ->get()->map(fn ($i) => ['type' => $i->engine_type ?? 'Not Specified', 'count' => $i->count, 'avg_horsepower' => round($i->avg_hp ?? 0, 2)])->toArray();

        return ['engine_types' => $engineTypes, 'most_common_engine' => $engineTypes[0] ?? null];
    }
    ...
    private function getBoatrFishingGearAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $byGear = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->whereNotNull('primary_fishing_gear')->where('primary_fishing_gear', '!=', '')
            ->select('primary_fishing_gear')
            ->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count')
            ->groupBy('primary_fishing_gear')->orderByDesc('count')
            ->get()->map(fn ($i) => ['gear' => $i->primary_fishing_gear, 'total' => $i->count, 'approved' => $i->approved_count,
                'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0])->toArray();

        return ['distribution' => $byGear, 'most_common' => collect($byGear)->sortByDesc('total')->take(3)->values()->toArray(), 'total_gear_types' => count($byGear)];
    }

    private function getBoatrInspectionAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $s = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->selectRaw('SUM(CASE WHEN inspection_completed = 1 THEN 1 ELSE 0 END) as completed, SUM(CASE WHEN inspection_completed = 0 OR inspection_completed IS NULL THEN 1 ELSE 0 END) as pending, COUNT(*) as total')
            ->first();

        return [
            'inspections_completed' => $s->completed ?? 0,
            'inspections_pending'   => $s->pending ?? 0,
            'total_applications'    => $s->total ?? 0,
            'completion_rate'       => $s->total > 0 ? round(($s->completed / $s->total) * 100, 2) : 0,
        ];
    }

    private function getBoatrApprovalAnalysis(Carbon $startDate, Carbon $endDate): array
    {
        $applications = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])->whereNotNull('reviewed_at')->get();
        ...
        return [
            ...
            'fastest_processing'      => $applications->isNotEmpty()
                ? round($applications->min(fn ($a) => $a->created_at->diffInHours($a->reviewed_at)), 2) . ' hours' : 'N/A',
            'slowest_processing'      => $applications->isNotEmpty()
                ? round($applications->max(fn ($a) => $a->created_at->diffInHours($a->reviewed_at)), 2) . ' hours' : 'N/A',
        ];
    }
    ...
    private function getBoatrMotorizedBreakdown(Carbon $startDate, Carbon $endDate): array
    {
        $distribution = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ...
            ->selectRaw('COUNT(*) as count, SUM(CASE WHEN status = "approved" THEN 1 ELSE 0 END) as approved_count, SUM(CASE WHEN inspection_completed = 1 THEN 1 ELSE 0 END) as inspections_completed, AVG(engine_horsepower) as avg_horsepower')
            ->groupBy('boat_classification')->orderByDesc('count')
            ->get()->map(fn ($i) => ['classification' => $i->boat_classification, 'total' => $i->count,
                'approved' => $i->approved_count, 'approval_rate' => $i->count > 0 ? round(($i->approved_count / $i->count) * 100, 2) : 0,
                'inspections_completed' => $i->inspections_completed, 'avg_horsepower' => round($i->avg_horsepower ?? 0, 2)])->toArray();

        $total = array_sum(array_column($distribution, 'total'));
        $distribution = array_map(fn ($item) => array_merge($item, ['percentage' => $total > 0 ? round(($item['total'] / $total) * 100, 1) : 0]), $distribution);
    ...
    private function getBoatrMultipleRegistrations(Carbon $startDate, Carbon $endDate): array
    {
        $multiple = BoatrApplication::whereBetween('created_at', [$startDate, $endDate])
            ->whereNotNull('fishr_number')->where('fishr_number', '!=', '')
            ->select('fishr_number', DB::raw('COUNT(*) as boat_count'))
            ->groupBy('fishr_number')->having('boat_count', '>', 1)->orderByDesc('boat_count')
            ->get()->map(fn ($i) => ['fishr_number' => $i->fishr_number, 'boat_count' => $i->boat_count])->toArray();
    ...
    // ─────────────────────────────────────────────────────────────────
    // PRIVATE HELPERS (supply-specific, unchanged)
    // ─────────────────────────────────────────────────────────────────

    private function getDetailedStockStatus($item): string
    {
        if ($item->current_supply <= 0) return 'Out of Stock';
        if ($item->minimum_supply && $item->current_supply <= $item->minimum_supply) return 'Critical Level';
        if ($item->reorder_point && $item->current_supply <= $item->reorder_point) return 'Needs Reorder';
        if ($item->current_supply <= 10) return 'Low Stock';
        if ($item->maximum_supply && $item->current_supply >= $item->maximum_supply * 0.8) return 'Well Stocked';
        return 'Adequate';
    }

    private function getStockStatusColor($item): string
    {
        if ($item->current_supply <= 0) return 'danger';
        if ($item->minimum_supply && $item->current_supply <= $item->minimum_supply) return 'danger';
        if ($item->reorder_point && $item->current_supply <= $item->reorder_point) return 'warning';
        if ($item->current_supply <= 10) return 'warning';
        return 'success';
    }
    ...
    {
        $outP = $count > 0 ? ($outOfStock / $count) * 100 : 0;
        $lowP = $count > 0 ? ($lowStock / $count) * 100 : 0;
        if ($outP >= 50) return 'Critical';
        if ($outP >= 25 || $lowP >= 50) return 'Poor';
        if ($lowP >= 25) return 'Fair';
        return 'Good';
    }

    private function getUrgencyLevel($item): string
    {
        if ($item->current_supply <= 0) return 'CRITICAL';
        if ($item->minimum_supply && $item->current_supply <= $item->minimum_supply) return 'HIGH';
        if ($item->reorder_point && $item->current_supply <= $item->reorder_point) return 'MEDIUM';
        return 'LOW';
    }

    private function getRecommendedAction($item): string
    {
        if ($item->current_supply <= 0) return 'Immediate procurement required - Item completely out of stock';
        if ($item->minimum_supply && $item->current_supply <= $item->minimum_supply) return 'Emergency restock needed - Below minimum safety level';
        if ($item->reorder_point && $item->current_supply <= $item->reorder_point) return 'Initiate regular reorder process';
    ...
    private function calculateSupplyHealthScore($supplyStats): int
    {
        ...
        $penalty    = (($supplyStats->out_of_stock_items ?? 0) * 10)
                    + (($supplyStats->low_stock_items ?? 0) * 5)
                    + (($supplyStats->critical_items ?? 0) * 8);
        $maxPenalty = $totalItems * 10;
        return $maxPenalty === 0 ? 100 : round(max(0, 100 - (($penalty / $maxPenalty) * 100)));
    }
    ...
    private function generateSupplySummary($supplyStats, $categoryDistribution, $attentionItems): array
    {
        ...
        $overallStatus = 'Good';
        if ($outOfStockPercent >= 25 || $criticalPercent >= 20) $overallStatus = 'Critical';
        elseif ($outOfStockPercent >= 10 || $lowStockPercent >= 30) $overallStatus = 'Poor';
        elseif ($lowStockPercent >= 15) $overallStatus = 'Fair';

        $topCategories    = $categoryDistribution->where('total_stock', '>', 0)->sortByDesc('total_stock')->take(3)->pluck('category_name')->toArray();
        $concernCategories= $categoryDistribution->where('stock_status', '!=', 'Good')->pluck('category_name')->toArray();
    ...
    private function generateSummaryText($status, $totalStock, $totalItems, $outOfStockPercent, $lowStockPercent): string
    {
        ...
        $summary = "Supply inventory contains {$totalStock} total units across {$totalItems} different item types (average {$stockDensity} units per item type). ";
        if ($status === 'Critical') $summary .= "CRITICAL STATUS: {$outOfStockPercent}% of items are out of stock. Immediate action required.";
        elseif ($status === 'Poor') $summary .= "Supply challenges identified with {$outOfStockPercent}% out of stock and {$lowStockPercent}% low stock items.";
        elseif ($status === 'Fair') $summary .= "Supply levels are manageable but {$lowStockPercent}% of items need restocking attention.";
        else $summary .= "Supply levels are generally adequate with good stock distribution.";
        return $summary;
    }
    ...
    private function calculateAverageProcessingTime($requests): float
    {
        ...
        if ($processedRequests->isEmpty()) return 0;
        $totalDays = $processedRequests->sum(fn ($r) => $r->created_at->diffInDays($r->updated_at));
